...
// Package utils contains internal helper functions for neatchain commands.
package utils
...
func MakeAddress(ks *keystore.KeyStore, account string) (accounts.Account, error) {
	...
	log.Warn("Please use explicit addresses! (can search via `neatchain account list`)")
	log.Warn("-------------------------------------------------------------------")
...
func RegisterIntService(stack *node.Node, cfg *neatptc.Config, cliCtx *cli.Context, cch core.CrossChainHelper) {
	...
	if err != nil {
		Fatalf("Failed to register the neatchain service: %v", err)
	}
}
...
func MakeChain(ctx *cli.Context, stack *node.Node) (chain *core.BlockChain, chainDb neatdb.Database) {
	...
	cache := &core.CacheConfig{
		TrieCleanLimit: neatptc.DefaultConfig.TrieCleanCache,

		TrieDirtyLimit:    neatptc.DefaultConfig.TrieDirtyCache,
		TrieDirtyDisabled: ctx.GlobalString(GCModeFlag.Name) == "archive",
		TrieTimeLimit:     neatptc.DefaultConfig.TrieTimeout,
	}
	if ctx.GlobalIsSet(CacheFlag.Name) || ctx.GlobalIsSet(CacheTrieFlag.Name) {
...
// new format.
//
// e.g. neatchain account new --keystore /tmp/mykeystore --lightkdf
//
// is equivalent after calling this method with:
//
// neatchain --keystore /tmp/mykeystore --lightkdf account new
//
