...
func TestIPCAttachWelcome(t *testing.T) {
	...
	if runtime.GOOS == "windows" {
		ipc = `\\.\pipe\neatchain` + strconv.Itoa(trulyRandInt(100000, 999999))
	} else {
		ws := tmpdir(t)
		defer os.RemoveAll(ws)
		ipc = filepath.Join(ws, "neatchain.ipc")
	}
	...
	testAttachWelcome(t, neatchain, "ipc:"+ipc, ipcAPIs)
...
func TestHTTPAttachWelcome(t *testing.T) {
	...
	time.Sleep(2 * time.Second) // Simple way to wait for the RPC endpoint to open
	testAttachWelcome(t, neatchain, "http://localhost:"+port, httpAPIs)
...
func TestWSAttachWelcome(t *testing.T) {
	...
	time.Sleep(2 * time.Second) // Simple way to wait for the RPC endpoint to open
	testAttachWelcome(t, neatchain, "ws://localhost:"+port, httpAPIs)
...
func testAttachWelcome(t *testing.T, neatchain *testintchain, endpoint, apis string) {
	...
	attach.SetTemplateFunc("datadir", func() string { return neatchain.Datadir })
