...
var (
	...
	// errInvalidMainChainNumber is returned when side chain block doesn't contain the valid main chain height
...
func (sb *backend) verifyHeader(chain consensus.ChainReader, header *types.Header, parents []*types.Header) error {
	...
	if header.Time.Cmp(big.NewInt(now().Unix())) > 0 {
		...
		//in neatchain, avoid the time difference to tolerate time gap between nodes
	...
	// Check the MainChainNumber if on Side Chain
...
func (sb *backend) VerifyHeaders(chain consensus.ChainReader, headers []*types.Header, seals []bool) (chan<- struct{}, <-chan error) {
	...
	sb.logger.Info("NeatCon is now verifying headers")
...
func (sb *backend) Prepare(chain consensus.ChainReader, header *types.Header) error {
	...
	// Add Main Chain Height if running on Side Chain
...
func (sb *backend) Finalize(chain consensus.ChainReader, header *types.Header, state *state.StateDB, txs []*types.Transaction,
	...
	// Check if any Side Chain need to be launch and Update their account balance accordingly
	if sb.chainConfig.NeatChainId == params.MainnetChainConfig.NeatChainId || sb.chainConfig.NeatChainId == params.TestnetChainConfig.NeatChainId {
		// Check the Side Chain Start
...
func prepareExtra(header *types.Header, vals []common.Address) ([]byte, error) {
	...
	//logger.Info("NeatCon (backend) prepare extra ")
...
func writeSeal(h *types.Header, seal []byte) error {
	...
	//logger.Info("NeatCon backend write seal")
...
func writeCommittedSeals(h *types.Header, ncExtra *ntcTypes.NeatConExtra) error {
	...
	//logger.Info("NeatCon backend write committed seals")
...
func accumulateRewards(config *params.ChainConfig, state *state.StateDB, header *types.Header, ep *epoch.Epoch, totalGasFee *big.Int) {
	...
	if config.NeatChainId == params.MainnetChainConfig.NeatChainId || config.NeatChainId == params.TestnetChainConfig.NeatChainId {
		...
		if rewardPerBlock != nil && rewardPerBlock.Sign() == 1 {
			...
			// sub balance from sideChainRewardAddress, reward per blocks
