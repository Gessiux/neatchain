...
func (n *Node) OnStop() {
	...
	//n.sw.StopChainReactor(n.consensusState.GetState().NTCExtra.ChainID)
	n.evsw.Stop()
...
//		uint64(state.NTCExtra.Height), block.NumberU64())
//
//	if uint64(state.NTCExtra.Height) != block.NumberU64() {
...
// should fork neatcon/neatcon and implement RunNode to
