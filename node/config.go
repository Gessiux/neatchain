...
import (
	...
	"github.com/Gessiux/neatchain/accounts/usbwallet"
...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of neatchain is "neatchain". If no
...
func (c *Config) NodeName() string {
	...
	//if name == "neatchain" || name == "neatchain-testnet" {
	//	name = "neatchain"
	//}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "neatchain" instances.
...
func (c *Config) ResolvePath(path string) string {
	...
	if c.name() == "neatchain" && isOldGethResource[path] {
		oldpath := ""
		if c.Name == "neatchain" {
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "neatchain-keystore")
		ephemeral = keydir
	}
