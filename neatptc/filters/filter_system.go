...
// Package filters implements an neatchain filtering system for block,
...
type subscription struct {
	...
	typ       Type
	created   time.Time
	logsCrit  neatchain.FilterQuery
	logs      chan []*types.Log
...
func (es *EventSystem) SubscribeLogs(crit neatchain.FilterQuery, logs chan []*types.Log) (*Subscription, error) {
...
func (es *EventSystem) subscribeMinedPendingLogs(crit neatchain.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
func (es *EventSystem) subscribeLogs(crit neatchain.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
func (es *EventSystem) subscribePendingLogs(crit neatchain.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
