...
var (
	...
	maxQueuedHeaders  = 32 * 1024 // [neatptc/62] Maximum number of headers to queue for import (DOS protection)
...
type Downloader struct {
	...
	// Channels
	headerCh      chan dataPack        // [neatptc/62] Channel receiving inbound block headers
	bodyCh        chan dataPack        // [neatptc/62] Channel receiving inbound block bodies
	receiptCh     chan dataPack        // [neatptc/63] Channel receiving inbound receipts
	bodyWakeCh    chan bool            // [neatptc/62] Channel to signal the block body fetcher of new tasks
	receiptWakeCh chan bool            // [neatptc/63] Channel to signal the receipt fetcher of new tasks
	headerProcCh  chan []*types.Header // [neatptc/62] Channel to feed the header processor new tasks

	// for stateFetcher
	stateSyncStart chan *stateSync
	trackStateReq  chan *stateReq
	stateCh        chan dataPack // [neatptc/63] Channel receiving inbound node state data
...
// these are zero.
func (d *Downloader) Progress() neatchain.SyncProgress {
